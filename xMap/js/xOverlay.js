...
*/
    ...
    */
    ...
    xCanvas.prototype.removeOverlay = function(overlay) {
...
*/
    ...
    */
    ...
    xCanvas.prototype.showTitle=function(circleOverlay)
    {
    ...
    */
    ...
    xCanvas.prototype.onMapMouseMove = function() {
        ...
        me.map.addEventListener("mousemove", function(type, target, point, pixel, overlay) {
            ...
            me.onMouseMove(me.SelectedOverlay, me.SelectedItem);
            me.initEdit(me.SelectedOverlay, me.SelectedItem)
        });
    }
    xCanvas.prototype.onMouseMove = function(selectedOverlay, selectedItem) {

    }
    xCanvas.prototype.getLevel = function() {
    ...
    xCanvas.prototype.draw = function(e, e1, e2, e3, e4, e5) {
            ...
            this.log(this.left + "," + this.top);
            this.totalExcuteTime("清除画布", function() {
                me.clear();
            });
            this.totalExcuteTime("重绘-draw", function() {
                me.onDraw();
            });
    ...
    xCanvas.prototype.refresh = function(onFilter) {
            this.clear();
            this.onDraw(onFilter);
        }
        /*
        重绘事件
        */
    xCanvas.prototype.onDraw = function(onFilter) {
    ...
    xCanvas.prototype.log = function(text) {
    ...
    xCanvas.prototype.totalExcuteTime = function(moduleName, onaction) {
        ...
        //返回函数执行需要时间
        if (window.console && window.console.log) {
            console.log("模块：" + moduleName + ", 执行时间:" + (end - start) + "ms" + ",now date:" + new Date().getTime());
        }
    }
    xCanvas.prototype.setSize = function(width, height) {
    ...
    xCanvas.prototype.drawImage = function(canvas, postions, onBefore, onCompleted, param, moduleName) {
        ...
        if (cxt) {
            ...
            if (param.width != param.image.naturalWidth) {
                param.width = param.image.naturalWidth;
                param.height = param.image.naturalHeight;
                param.offsetTop = param.height;
                param.offsetLeft = param.width / 2;
            ...
            (function(img, x, y, cxt) {
                cxt.save();
                cxt.globalAlpha = 1;
                var newimg = new Image();
                newimg.src = img.src;
                newimg.onload = function() {
                    cxt.drawImage(newimg, x, y);
                    newimg = null;
                    cxt.restore();
                }

            })(param.image, _x, _y, cxt)
            // img.src = param.image.src;
            //img.onload = function() {

            //}
            //img=null;
            this.DrawText(param, cxt);
            if (onCompleted) {
    ...
    xCanvas.prototype.isEdit = false;

    xCanvas.prototype.editStart = function(selectedItem) {
        this.isEdit = true;

    }
    xCanvas.prototype.setVisiable = function(item, isVisable) {
        item['visiable'] = isVisable;
    }
    xCanvas.prototype.setVisiable_byName = function(selectedItem, isVisable) {
        //item['visiable']=isVisable;
        //if(indexOf(item['name'],item_name)>=0)
        // item['visiable']=isVisable;
        //  alert(item['name']);

    }

    xCanvas.prototype.isVisable = function(item) {
        return item['visiable'];
    }
    xCanvas.prototype.editEnd = function(selectedItem) {
        ...
        this.SelectedOverlay.disableDragging();


        this.onEditEnd(clone(this.SelectedItem), this.SelectedOverlay);
        this.SelectedOverlay = null;
        this.SelectedItem = null;
        this.clear();
        this.setDisplay(selectedItem, true);
        this.onDraw();
    }

    xCanvas.prototype.enableEdit = false;
    xCanvas.prototype.onEditBegin = function() {}
    xCanvas.prototype.onEditEnd = function(selectedItem, selectedOverlay) {


    }
    xCanvas.prototype.initEdit = function(selectedOverlay, selectedItem) {

        if (!this.enableEdit && selectedOverlay && selectedItem) {
            return;
        }
        var me = this;
        var canvaMe = this;
        if (selectedItem && selectedOverlay) {
            this.isEdit = true;

            selectedOverlay.enableDragging();
            selectedOverlay.addEventListener('dragstart', function(e) {
                me.editStart(selectedItem);
            });
            selectedOverlay.addEventListener('remove', function(e) {
                me.editEnd(selectedItem);

            });
            selectedOverlay.addEventListener('dragend', function(e) {

                var lat = e.point.lat;
                var lng = e.point.lng;
                if (selectedItem.overlayType == xOverlay.Types.Image) {
                    selectedItem.lat = lat;
                    selectedItem.lng = lng;
                    me.removeOverlay(selectedOverlay)
                }
                return false;
            })
        }
 ...
 */
    ...
    xCanvas.prototype.DrawText = function(tmpl, cxt) {
        ...
        if (!tmpl || !tmpl.name || !tmpl.isShowTitle) {
            return;
        }
        if (tmpl.nameWidth) {
        ...
        var nameDesc = tmpl[xFields.Name];
        if (!nameDesc) {
            return;
        }
       ...
       cxt.save();
        cxt.font = '12px';
        var left = tmpl.x - (cxt.measureText(nameDesc).width/2) ;
        cxt.fillText(nameDesc, left, tmpl.y);
        cxt.restore();
            ...
            if (tmpl.innerRings && innerRings.length > 0) {
                for (var i = 0; i < innerRings.length; i++) {
                    ...
                    this.drawPolyline(canvas, points, function(tmpl, ctx) {
                        ...
                        ctx.globalAlpha = 1;
                    }, function() {}, {}, true, requestModule.name);
                    this.drawPolyline(canvas, points, function(tmpl, ctx) {
                            //ctx.globalCompositeOperation = "destination-out";
                            //    ctx.globalAlpha = 0.1;
 ...
 */
        ...
        createCircle: function(opts, radius) {
            if(opts instanceof BMap.Point){
                return   new BMap.Circle(opts, radius,{strokeColor:"blue", strokeWeight:6, strokeOpacity:0.5});
            }
